...
1. `handoffs/planner-state.md` — run state, revision 5. Its "The gate" and "HEADLINE FINDING"
...
three consecutive gates before anyone caught it. Several distinct false-green classes are enumerated
in the state file. What binds you:
...
- **When two readings of a number differ, prefer the one that fails loudly.** A false alarm costs a
  ...
  choosing the quieter reading — including one of mine, where I claimed a FULL gate on the feature
  tree "correctly reports 136". **It does not. The expectation is 157**, because
  `compileTestKotlin dependsOn(generateTests)` and `generateTests` declares `inputs.dir(testData)`,
  so the gate regenerates the runner against the on-disk test files before compiling. A 136 on that
  tree is the signature that the new cases were never wired in and none of them ran.
...
`e1cd7c1c` measured **11m39s**; runs carrying new Silicon-invoking cases will be slower. `flock`
gives **no fairness guarantee**, so queue position is not FIFO and the tail is unbounded in
principle. The two-attempt budget is **structural, not hardening**: only a bound makes an unbounded
wait reportable.
...
Require the diagnostic text, not the exit status.** A permission-ground rejection where a predicate
ground was expected is a false negative control that reads as success.
...
- `PhasedDiagnosticTestGenerated.java` is **generated but tracked and not gitignored**. The repo
  keeps it in step with its inputs (111↔111 at `bf32366c`, 116↔116 at `e1cd7c1c`), so
  `committed runTest count == committed testData .kt count` is a cheap positive check **against the
  object store**, not against a post-build working tree. **The final single regeneration is a merge
  gate owned by the Launcher, not a worker task.**
...
- **Never `git stash`** — `refs/stash` is one ref shared by every worktree. Use `mv` into `/tmp`.
...
- Write your state file **incrementally as you go**, not at handover, and report at **130k**
  context. Do not run to the wall. Trust your turn footer over your sense of your own usage — three
  agents in this run misjudged it by 25k+.
- Spawn any worker with an **explicit model**, and brief it to tag turn-ends; untagged turn-ends
  reach nobody.
- **Label every rule you hand a subordinate structural or hygiene, and resolve ambiguous cases as
  structural.** A hygiene rule mistaken for structural costs time; a structural rule mistaken for
  hygiene costs the result.

## Reporting

Report to the Planner seat at step boundaries, tagging turns `%notify: spawner`. Everything reaching
the operator goes through the Launcher via the Planner — do not attempt direct contact. When you
catch your own error, say so **with the remediation attached, in the same turn**. Those self-catches
are the only reason this run is recoverable.
